...
RefPtr<QueryTreeNode> QueryPlanBuilder::build(
  ...
  if ((node = buildDropPartition(txn, ast)) != nullptr) {
    return node;
  }
...
QueryTreeNode* QueryPlanBuilder::buildDropPartition(
    Transaction* txn,
    ASTNode* ast) {
  if (!(*ast == ASTNode::T_DROP_PARTITION) || ast->getChildren().size() != 2) {
    return nullptr;
  }
