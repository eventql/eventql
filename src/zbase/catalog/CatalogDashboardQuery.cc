 ...
 */
#include "CatalogDashboardQuery.h"
...
CatalogDashboardQuery::CatalogDashboardQuery(
  ...
  query->onSession(std::bind(&CatalogDashboardQuery::onSession, this));
  query->onQuery(std::bind(&CatalogDashboardQuery::onQuery, this));
  ...
  String catid;
  if (URI::getParam(params.params, "category1", &catid)) {
    auto col = query->fetchColumn("search_queries.category1");
    auto cid = std::stoul(catid);
    filter_fn_ = [col, cid] { return col->getUInt32() == cid; };
  } else if (URI::getParam(params.params, "category2", &catid)) {
    auto col = query->fetchColumn("search_queries.category2");
    auto cid = std::stoul(catid);
    filter_fn_ = [col, cid] { return col->getUInt32() == cid; };
  } else if (URI::getParam(params.params, "category3", &catid)) {
    auto col = query->fetchColumn("search_queries.category3");
    auto cid = std::stoul(catid);
    filter_fn_ = [col, cid] { return col->getUInt32() == cid; };
  } else {
    filter_fn_ = [] { return true; };
  }

  String drilldown_type = "device_type";
...
void CatalogDashboardQuery::onSession() {
  new_session_ = true;
}

void CatalogDashboardQuery::onQuery() {
  ...
  if (pagetype != PageType::CATALOG_PAGE || !filter_fn_()) {
    return;
  }
...
RefPtr<AnalyticsQueryResult::SubQueryResult> CatalogDashboardQuery::result() {
  return result_.get();
}

void CatalogDashboardQuery::setDrilldownFn(
    const String& type,
    AnalyticsTableScan* query) {
  if (type == "time") {
