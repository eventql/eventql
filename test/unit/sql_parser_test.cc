...
TEST_CASE(ParserTest, TestCreateTableWithStatement, [] () {
  ...
  EXPECT_EQ(stmt->getChildren()[0]->getToken()->getString(), "fnord");
  EXPECT(*stmt->getChildren()[1] == ASTNode::T_COLUMN_LIST);
  EXPECT(stmt->getChildren()[1]->getChildren().size() == 2);

  EXPECT_EQ(*stmt->getChildren()[2], ASTNode::T_TABLE_PROPERTY_LIST);
  auto property_list = stmt->getChildren()[2]->getChildren();
  EXPECT_EQ(property_list.size(), 2);
  EXPECT_EQ(*property_list[0], ASTNode::T_TABLE_PROPERTY);
  EXPECT_EQ(property_list[0]->getChildren().size(), 2);
  EXPECT_EQ(
      *property_list[0]->getChildren()[0],
      ASTNode::T_TABLE_PROPERTY_KEY);
  EXPECT_EQ(
      *property_list[0]->getChildren()[0]->getToken(),
      Token::T_IDENTIFIER);
  EXPECT_EQ(
      property_list[0]->getChildren()[0]->getToken()->getString(),
      "blah");
  EXPECT_EQ(
      *property_list[0]->getChildren()[1],
      ASTNode::T_LITERAL);
  EXPECT_EQ(
      *property_list[0]->getChildren()[1]->getToken(),
      Token::T_STRING);
  EXPECT_EQ(
      property_list[0]->getChildren()[1]->getToken()->getString(),
      "my_value");

  EXPECT_EQ(
      *property_list[1]->getChildren()[0],
      ASTNode::T_TABLE_PROPERTY_KEY);
  EXPECT_EQ(
      *property_list[1]->getChildren()[0]->getToken(),
      Token::T_IDENTIFIER);
  EXPECT_EQ(
      property_list[1]->getChildren()[0]->getToken()->getString(),
      "foo.bar");
  EXPECT_EQ(
      *property_list[1]->getChildren()[1],
      ASTNode::T_LITERAL);
  EXPECT_EQ(
      *property_list[1]->getChildren()[1]->getToken(),
      Token::T_STRING);
  EXPECT_EQ(
      property_list[1]->getChildren()[1]->getToken()->getString(),
      "other value");
});
