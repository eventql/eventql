...
#include <algorithm>
#include <thread>
#include <fnord-doctable/TableWriter.h>
#include <fnord-doctable/TableSegmentSummaryWriter.h>
...
void TableWriter::merge() {
...

  {
    RefPtr<TableSegmentWriter> writer(
        new TableSegmentWriter(db_path_, name_, &schema_, &output_chunk));

    for (const auto& summary_factory : summaries_) {
      writer->addSummary(summary_factory());
    }

    TableSegmentMerge mergeop(
        db_path_,
        name_,
        &schema_,
        input_chunks,
...
void TableWriter::writeTable(RefPtr<TableArena> arena) {
  ...
  {
    TableSegmentWriter writer(db_path_, name_, &schema_, &chunk);
    for (const auto& summary_factory : summaries_) {
      writer.addSummary(summary_factory());
    }
...
void TableWriter::addChunk(const TableSegmentRef* chunk, ArtifactStatus status) {
...
TableSegmentWriter::TableSegmentWriter(
    const String& db_path,
    const String& table_name,
...
void TableSegmentWriter::addSummary(RefPtr<TableSegmentSummaryBuilder> summary) {
  summaries_.emplace_back(summary);
}

void TableSegmentWriter::addRecord(const msg::MessageObject& record) {
  Buffer buf;
...
void TableSegmentWriter::commit() {
  fnord::logInfo("fnord.evdb", "Writing chunk: $0", chunk_name_);

  cstable_->write(chunk_filename_ + ".cst~");
  sstable_->finalize();

  TableSegmentSummaryWriter summary_writer(chunk_filename_ + ".smr~");
  for (auto& s : summaries_) {
...
TableSegmentMerge::TableSegmentMerge(
    const String& db_path,
    const String& table_name,
    msg::MessageSchema* schema,
...
void TableSegmentMerge::readTable(const String& filename) {
...
bool TableMergePolicy::findNextMerge(
  ...
  std::sort(chunks.begin(), chunks.end(), [] (
      const TableSegmentRef& a,
      const TableSegmentRef& b) -> bool {
