...
const std::vector<std::pair<std::string, SType>> ShowDatabasesExpression::kOutputColumns = {
  { "database_name", SType::STRING }
};
...
ReturnCode ShowDatabasesExpression::execute() {
  auto rc = txn_->getTableProvider()->listDatabases(
      [this] (const eventql::NamespaceConfig& ns) {
    Vector<SValue> row;
    row.emplace_back(SValue::newString(ns.customer()));
    addRow(row.data());
  });
